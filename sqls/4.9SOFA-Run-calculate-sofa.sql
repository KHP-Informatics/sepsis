...
DROP  VIEW IF EXISTS sepsis.SOFA CASCADE;
CREATE VIEW sepsis.SOFA AS
with wt AS
(
...
, echo2 as(
  ...
  from icustays ie
  left join sepsis.echodata echo
...
(
  ...
  from sepsis.bloodgasfirstdayarterial bg
  left join sepsis.ventdurations vd
...
left join sepsis.vitalsfirstday v
  on ie.icustay_id = mv.icustay_id
left join sepsis.labsfirstday l
  on ie.icustay_id = l.icustay_id
left join sepsis.uofirstday uo
  on ie.icustay_id = uo.icustay_id
left join sepsis.gcsfirstday gcs
...
drop TABLE if exists sepsis.sofa_distinct cascade; 

create TABLE sepsis.sofa_distinct as
select distinct subject_id, hadm_id, icustay_id, sofa, respiration, coagulation, liver, cardiovascular, cns, renal from sepsis.sofa; 
